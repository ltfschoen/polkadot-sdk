...
//! Integration tests for origin-and-gate pallet focusing on proposal lifecycle.

use assert_matches::assert_matches;
use frame_support::{assert_ok, BoundedVec};
use frame_support::traits::{EnsureOrigin, PalletInfoAccess};
use pallet_origin_and_gate::{Approvals, Config, ProposalInfo, ProposalStatus, Proposals, REQUIRED_APPROVALS};
use sp_core::H256;
...
fn proposal_is_approved_but_does_not_execute_and_status_remains_pending_when_only_proposed() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(proposal.approvals.len(), 1); // Only Alice has approved
		...
		assert!(events.iter().any(|record| matches!(
			record.event,
			RuntimeEvent::OriginAndGate(pallet_origin_and_gate::Event::ProposalCreated { proposal_hash, .. })
			if proposal_hash == call_hash
		)));

		// Verify that no `ProposalExecuted` event was emitted
		assert!(!events.iter().any(|record| matches!(
			record.event,
			RuntimeEvent::OriginAndGate(pallet_origin_and_gate::Event::ProposalExecuted { .. })
		)));
	});
}
...
fn proposal_successfully_executes_and_status_becomes_executed_with_two_approvals() {
	new_test_ext().execute_with(|| {
		...
		assert!(events.iter().any(|record| matches!(
			record.event,
			RuntimeEvent::OriginAndGate(pallet_origin_and_gate::Event::ProposalApproved { proposal_hash, .. })
			if proposal_hash == call_hash
		)));
