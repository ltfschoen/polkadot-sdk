	...
	mod cancel_proposal {
		...
		fn proposal_cancellation_only_by_proposer_and_only_when_pending_status() {
			new_test_ext().execute_with(|| {
				...
				let call_call_hash2 = <Test as Config>::Hashing::hash_of(&call2);
				...
				Proposals::<Test>::insert(call_call_hash2, ALICE_ORIGIN_ID, proposal_info);

				// Verify proposal remains `Executed`
				let proposal = Proposals::<Test>::get(call_call_hash2, ALICE_ORIGIN_ID).unwrap();
	...
	mod race_condition {
		...
		fn test_race_condition_approval_after_execution() {
			new_test_ext().execute_with(|| {
				...
				// Bob approves, which should trigger execution
				assert_ok!(OriginAndGate::add_approval(
					RuntimeOrigin::signed(BOB),
					call_hash,
					ALICE_ORIGIN_ID,
				...
				// Charlie tries to approve after execution
				assert_noop!(
					OriginAndGate::add_approval(
						RuntimeOrigin::signed(CHARLIE),
						call_hash,
		...
		fn test_race_condition_execution_after_expiry() {
			new_test_ext().execute_with(|| {
				...
				// Create a proposal with expiry at block 10
				let call = make_remark_call("1000").unwrap();
				let call_hash =
					<<Test as Config>::Hashing as sp_runtime::traits::Hash>::hash_of(&call);
				...
				// Fast forward to block 11 (expired)
				System::set_block_number(11);

				// Check the proposal status before attempting approval
				...
				// Bob tries to approve after expiry
				let result = OriginAndGate::add_approval(
					RuntimeOrigin::signed(BOB),
					call_hash,
					ALICE_ORIGIN_ID,
				...
				// Verify the call fails with ProposalExpired error
				assert_err!(result, Error::<Test>::ProposalExpired);

				// Since the transaction failed, the proposal status should still be Pending
				// because the status update is rolled back when the transaction fails
				...
				// Note: We cannot verify the ProposalExpired event because events are rolled back
				// when a transaction fails in the Substrate test framework
			});
		}
	}
}
...
/// Tests specifically verifying technical requirements for AndGate
mod andgate_requirements {
	...
	fn andgate_requires_asynchronous_origin_approval() {
		new_test_ext().execute_with(|| {
			...
			// Create a call that requires both Alice and Bob's approval
			let call = create_dummy_call(1000);
			let call_hash = <Test as Config>::Hashing::hash_of(&call);

			// Step 1: Alice proposes and approves
			assert_ok!(OriginAndGate::propose(
				RuntimeOrigin::signed(ALICE),
				call.clone(),
				ALICE_ORIGIN_ID,
			...
			// Verify proposal is pending and not executed yet (only one approval)
			let proposal = Proposals::<Test>::get(call_hash, ALICE_ORIGIN_ID).unwrap();
			assert_eq!(proposal.status, ProposalStatus::Pending);
			...
			// Step 2: Time passes (simulate different block)
			System::set_block_number(2);
			let execution_timepoint = current_timepoint();

			// Ensure timepoints are different
			assert_ne!(alice_submission_timepoint, execution_timepoint);

			// Step 3: Bob comes along later and approves
			assert_ok!(OriginAndGate::add_approval(
				RuntimeOrigin::signed(BOB),
				call_hash,
				ALICE_ORIGIN_ID,
			...
			// Verify ExecutedCalls storage is updated with the call hash at execution timepoint
	...
	fn andgate_retains_state_between_origin_approvals() {
		new_test_ext().execute_with(|| {
			...
			// Step 2: Simulate system restart or state reset (except storage)
			// by advancing blocks and clearing events
			System::set_block_number(100);
			System::reset_events();
			let execution_timepoint = current_timepoint();

			// Ensure timepoints are different (must be, since blocks advanced)
			assert_ne!(alice_submission_timepoint, execution_timepoint);

			// Step 3: Verify state is retained even after "system restart"
			let proposal = Proposals::<Test>::get(call_hash, ALICE_ORIGIN_ID).unwrap();
			assert_eq!(proposal.status, ProposalStatus::Pending);
			assert_eq!(proposal.approvals.len(), 1);
			assert_eq!(proposal.approvals[0], ALICE_ORIGIN_ID);
			...
			// Step 4: Bob can still approve the proposal later
			assert_ok!(OriginAndGate::add_approval(
				RuntimeOrigin::signed(BOB),
				call_hash,
				ALICE_ORIGIN_ID,
	...
	fn andgate_direct_synchronous_approval_should_fail() {
		new_test_ext().execute_with(|| {
			...
			// Create a call that would typically require AliceAndBob origin
			let call = create_dummy_call(1000);
			let call_hash = <Test as Config>::Hashing::hash_of(&call);

			// Direct attempt using AndGate should fail
			// Note: This test explicitly validates that a synchronous approach fails,
			// showing why the asynchronous proposal system is necessary
			assert_noop!(
			...
			// Verify no ExecutedCalls entry was created since synchronous approach failed
	...
	fn andgate_as_ensureorigin_should_function_asynchronously() {
		new_test_ext().execute_with(|| {
			...
			// This test validates that AndGate implements EnsureOrigin but through
			// the asynchronous proposal mechanism, not direct origin checks

			// Create a call
			let call = create_dummy_call(1000);
			let call_hash = <Test as Config>::Hashing::hash_of(&call);

			// Step 1: Submit with first origin
			assert_ok!(OriginAndGate::propose(
				RuntimeOrigin::signed(ALICE),
				call.clone(),
				ALICE_ORIGIN_ID,
			...
			// Step 2: Attempt to execute with just one origin should fail
			let proposal = Proposals::<Test>::get(call_hash, ALICE_ORIGIN_ID).unwrap();
			assert_eq!(proposal.status, ProposalStatus::Pending);
			assert_eq!(Dummy::<Test>::get(), None);
			...
			// Move to next block for approval
			System::set_block_number(2);
			let execution_timepoint = current_timepoint();
			...
			// Step 3: Approve with second origin, allowing execution
			assert_ok!(OriginAndGate::add_approval(
				RuntimeOrigin::signed(BOB),
				call_hash,
				ALICE_ORIGIN_ID,
	...
	fn andgate_fulfills_technical_specification_requirements() {
		new_test_ext().execute_with(|| {
			// This test explicitly validates all requirements from the technical spec:
			// 1. Two independent EnsureOrigin implementations must both agree
			// 2. Origins cannot be collected together (asynchronous workflow)
			...
			// Create two different calls to test with
			...
			// Step 1: Alice proposes first call
			assert_ok!(OriginAndGate::propose(
				RuntimeOrigin::signed(ALICE),
				call1.clone(),
				ALICE_ORIGIN_ID,
			...
			// Move to block 5 for Bob's proposal
			...
			// Step 2: Bob proposes second call
			assert_ok!(OriginAndGate::propose(
				RuntimeOrigin::signed(BOB),
				call2.clone(),
				BOB_ORIGIN_ID,
			...
			// Ensure execution timepoint is different from submission timepoints
			...
			// Step 3: Alice approves Bob's proposal
			assert_ok!(OriginAndGate::add_approval(
				RuntimeOrigin::signed(ALICE),
				call_hash2,
				BOB_ORIGIN_ID,
			...
			// Move to a new block for the second approval
			System::set_block_number(11);
			let second_execution_timepoint = current_timepoint();

			// Ensure second execution timepoint is different
			assert_ne!(first_execution_timepoint, second_execution_timepoint);

			// Step 4: Bob approves Alice's proposal
			assert_ok!(OriginAndGate::add_approval(
				RuntimeOrigin::signed(BOB),
				call_hash1,
				ALICE_ORIGIN_ID,
			...
			assert_eq!(Dummy::<Test>::get(), Some(1000)); // Last executed call wins (NOTE: this seems to conflict with comment below, need to
												 // verify)
